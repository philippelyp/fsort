...
# fsort_sub is used to sort chunks in parallel
function fsort_sub
{
    filename=$1
...
if [ "$#" -ne 5  -a "$#" -ne 6 ]; then
	echo ""
	echo "Fast Sort v1.2"
	echo "Copyright 2018-2019 Philippe Paquet"
	echo ""
	echo "Usage:	fsort.sh <input_file> <output_file> <temporary_directory> <lines_per_chunk> <sort_buffer_size> <number_of_processes>"
	echo ""
	...
	echo "										Temporary files will consume 2 to 3 time the size of the input file"
	echo "										For example, if your input file is 100GB you should plan for 300GB to be available"
	echo ""
	...
	echo "										By default, fsort will use one process per logical core available"
	echo ""
	echo "Examples:	./fsort.sh file file.sort /var/temp/ 1000000 256M"
	echo "			./fsort.sh file file.sort /var/temp/ 2000000 512M 8"
	echo ""
	exit
fi
...
LC_ALL=C split -a 4 -l${lines_per_chunk} ${input_file} ${chunk_prefix};

# Sort chunks
ls ${chunk_filter} | xargs -P ${processes} -n 1 -I % bash -c 'fsort_sub %' _ {}

# Merge resulting chunks
LC_ALL=C sort $merge_flags -m  ${chunk_filter} -o ${output_file}
